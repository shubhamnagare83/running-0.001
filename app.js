...
const firebaseConfig = {
  apiKey: "AIzaSyCE5l69OEcEabSeTzASSGurCAp14pMTkLY",
  authDomain: "stride-84456.firebaseapp.com",
  projectId: "stride-84456",
  storageBucket: "stride-84456.firebasestorage.app",
  messagingSenderId: "134002448438",
  appId: "1:134002448438:web:1e61ab29ff448a1f622097",
  measurementId: "G-H3LQFD5ZB0"
};
...
try {
  ...
  console.log("STRIDE: Firebase initialized successfully");
} catch (e) {
  console.error("STRIDE: Firebase initialization failed, running in local-only mode", e);
}
...
async function saveRunToFirestore(run) {
  ...
  try {
    ...
    console.error("Failed to upload run to Firestore:", e);
    throw e;
  }
}

async function deleteRunFromFirestore(id) {
  if (!firebaseInitialized || !db) return;
  try {
    await deleteDoc(doc(db, "runs", id));
  } catch (e) {
    console.error("Failed to delete run from Firestore:", e);
    throw e;
  }
}
...
async function syncWithFirebase() {
  ...
  try {
    ...
    querySnapshot.forEach((doc) => {
      fbRuns.push(doc.data());
    });

    const localRuns = Storage.getLocalOnly();
    const runsMap = new Map();

    // Fill with cloud runs first
    fbRuns.forEach(r => runsMap.set(r.id, r));

    // Merge local runs (local updates/newer runs take precedence)
    let hasNewLocalRuns = false;
    for (const r of localRuns) {
      if (!runsMap.has(r.id)) {
        runsMap.set(r.id, r);
        // Upload local run that isn't on the cloud yet
        try {
          await saveRunToFirestore(r);
        } catch (err) {
          hasNewLocalRuns = true;
        }
      }
    }

    const mergedRuns = Array.from(runsMap.values());
    Storage.saveLocalOnly(mergedRuns);
...
const Storage = {
  ...
  getRuns(){
    return Storage.getLocalOnly();
  },
  saveRuns(runs){
    Storage.saveLocalOnly(runs);
  },
  async addRun(run){
  ...
  async deleteRun(id){
    ...
    if (firebaseInitialized && db) {
      updateSyncStatus('syncing', 'Syncing');
      try {
        await deleteRunFromFirestore(id);
        updateSyncStatus('synced', 'Synced');
      } catch (err) {
        updateSyncStatus('error', 'Local Only');
      }
